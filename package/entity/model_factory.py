...
def evaluate_classification_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6)->MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            train_acc = accuracy_score(y_train,y_pred=y_train_pred)
            test_acc = accuracy_score(y_test,y_pred=y_test_pred)
            
            #Calculating recall score on training and testing dataset
            train_recall = recall_score(y_train,y_pred=y_train_pred)
            test_recall = recall_score(y_test,y_pred=y_test_pred)

             #Calculating f1 score on training and testing dataset
            train_acc = f1_score(y_train,y_pred=y_train_pred)
            test_acc = f1_score(y_test,y_pred=y_test_pred)
           ...
           #logging all important metric
            ...
            if model_accuracy >= base_accuracy and diff_test_train_acc < 0.25:
