...
MetricInfoOrdner = namedtuple("MetricInfoOrdner",
...
def evaluate_classification_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6) -> MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            logging.info(f"Train Recall Score: [{train_recall}].")
            ...
            if model_accuracy >= base_accuracy and diff_test_train_acc < 0.10:
                ...
                metric_info_ordner = MetricInfoOrdner(model_name=model_name,
                                                        model_object=model,
                ...
                logging.info(f"Acceptable model found {metric_info_ordner}. ")
            index_number += 1
        if metric_info_ordner is None:
            logging.info(f"No model found with higher accuracy than base accuracy")
        return metric_info_ordner
...
class ModelFactory:
    def __init__(self, model_ordner_path: str = None,):
        try:
            ...
            self.grid_search_class_name: str = self.schmuck[GRID_SEARCH_KEY][CLASS_KEY]
            self.grid_search_property_data: dict = dict(self.schmuck[GRID_SEARCH_KEY][PARAM_KEY])

            self.models_initialization_config: dict = dict(self.schmuck[MODEL_SELECTION_KEY])
